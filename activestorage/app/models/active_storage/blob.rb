...
class ActiveStorage::Blob < ActiveRecord::Base
  ...
  class << self
    ...
    def build_after_upload(io:, filename:, content_type: nil, metadata: nil, key_format: nil, identify: true)
      new.tap do |blob|
        blob.filename     = filename
        blob.content_type = content_type
        blob.metadata     = metadata
        blob.key_format   = key_format
    ...
    # When providing a content type, pass <tt>identify: false</tt> to bypass automatic content type inference.
    def create_after_upload!(io:, filename:, content_type: nil, metadata: nil, key_format: nil, identify: true)
      build_after_upload(io: io, filename: filename, content_type: content_type, metadata: metadata, identify: identify, key_format: key_format).tap(&:save!)
    end
    ...
    # the signed ID.
    def create_before_direct_upload!(filename:, byte_size:, checksum:, content_type: nil, metadata: nil, key_format: nil)
      create! filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type, metadata: metadata, key_format: key_format
    end
  end
